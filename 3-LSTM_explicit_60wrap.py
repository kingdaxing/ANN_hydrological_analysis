...
def loss_and_error_subplot(history, title):
    ...
    # loss
    plt.subplot(2, 1, 1)
    plt.plot(epochs, loss, "orange", label="Training")
    ...
    # error
    plt.subplot(2, 1, 2)
    plt.plot(epochs, error, "orange", label="Training abs_Error")
    plt.plot(epochs, val_abs_error, "c", label="Validation abs_Error")
    plt.xlabel("Epochs")
    plt.ylabel("abs_Error")
    plt.subplots_adjust(hspace=0.5) # adjust the space between two subplots
    plt.tight_layout() 
    plt.show()

loss_and_error_subplot(history, "Loss and Error (wrapped by 60-min)")
